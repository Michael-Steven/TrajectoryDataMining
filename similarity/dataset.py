...
class Dataset:
    def __init__(self, src_file_path, trg_file_path, max_len, n_views):
        ...
        print(len(self.src_data), len(self.trg_data))
    ...
    def __getitem__(self, i):
        ...
        # while self.src_data_len[i][j] < 10 or self.trg_data_len[i][j] < 10:
        #     i = (i + 1) % len(self.src_data)
        #     j = np.random.randint(len(self.src_data[i]))
...
def get_dataloader(dataset_dir, max_len, batch_size, n_views=20, shuffle=True, num_workers=4):
    ...
    train_src_path = Path(dataset_dir) / "train_geolife.src"
    train_trg_path = Path(dataset_dir) / "train_geolife.trg"
    ...
    val_src_path = Path(dataset_dir) / "val_geolife.src"
    val_trg_path = Path(dataset_dir) / "val_geolife.trg"
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, drop_last=True)
