...
class GRUEncoder(nn.Module):
    ...
    def forward(self, trajs_hidden, trajs_len):
        ...
        # _, (hn, _) = self.lstm(packed_trajs_hidden)
        # outputs, _ = self.lstm(packed_trajs_hidden)
        outputs, _ = self.gru(trajs_hidden)
        # hn = hn.transpose(0, 1).reshape(trajs_hidden.shape[0], -1)
        # outputs: batch_size * 2 * n_views, seq_len, hidden_size * n_direction
        # outputs, _ = self.lstm(trajs_hidden)
        # hn: batch_size * 2 * n_views, hidden_size * n_direction
...
class TCNEncoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, bidirectional, n_layers, batch_first=True):
        ...
        self.bidirectional = bidirectional
        self.n_layers = n_layers
        self.tcn = TemporalConvNet(num_inputs=input_size, num_channels=[32]*2+[hidden_size], kernel_size=3, dropout=0)

    def forward(self, trajs_hidden, trajs_len):
        # trajs_hidden: batch_size * 2, seq_len, hidden_size
        # trajs_len: batch_size * 2
        # print(trajs_hidden.shape, trajs_len.shape)
        outputs = self.tcn(trajs_hidden.transpose(1, 2), trajs_len)
        # print(outputs.shape)
        return outputs
...
class AttnEncoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, bidirectional, n_layers, batch_first=True):
        ...
        self.bidirectional = bidirectional
        self.n_layers = n_layers
        self.pos_emb = PositionalEmbedding(hidden_size, 100)
        self.attn_encoder = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=4)
    ...
    def forward(self, trajs_hidden, trajs_len):
        ...
        # trajs_len: batch_size * 2
        # print(trajs_hidden.shape, trajs_len.shape)
        trajs_hidden = trajs_hidden + self.pos_emb(trajs_hidden)
        trajs_hidden = trajs_hidden.transpose(0, 1)
        outputs, _ = self.attn_encoder(trajs_hidden, trajs_hidden, trajs_hidden)
        outputs = outputs.permute(1, 2, 0)
        # print(outputs.shape)
        outputs = self.pooling(outputs).squeeze(-1)
        # outputs = outputs[torch.arange(trajs_len.shape[0]), trajs_len - 1]
        # print(outputs.shape)
        return outputs
...
class LSTMSimCLR(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size, bidirectional, n_layers):
        ...
        # self.encoder = GRUEncoder(self.hidden_size, self.hidden_size, self.bidirectional, self.n_layers)
        self.encoder = TCNEncoder(self.hidden_size, self.hidden_size, self.bidirectional, self.n_layers)
        # self.encoder = AttnEncoder(self.hidden_size, self.hidden_size, self.bidirectional, self.n_layers)
...
class BaseSimCLR(object):
    ...
    def __init__(self, model, args, optimizer, scheduler):
        ...
        logging.basicConfig(
            filename=os.path.join(
                args.logdir,
                'training_hiddensize_{}_batchsize_{}_bidirectional_{}_nlayers_{}_freeze_{}.log'.format(
                    args.hidden_size, args.batch_size, args.bidirectional, args.n_layers, args.freeze)
            ), level=logging.DEBUG
        )
    ...
    def info_nce_loss(self, features, batch_size):
        ...
        # print(positives.transpose(0, 1))
        # print(positives.shape, labels.shape)

        # select only the negatives the negatives
        negatives = similarity_matrix[~labels.bool()].view(similarity_matrix.shape[0], -1)
        # print(negatives)
        # print(negatives.shape, labels.shape)
    ...
    def train(self, train_loader, val_dataloader=None):
        ...
        for epoch_counter in range(self.args.epochs):
            ...
            for batch in tqdm(train_loader):
                ...
                # print(trajs.shape, trajs_len.shape)
                ...
                # if torch.isnan(loss).sum() > 0:
                    # print("here!")
                    # exit(0)
                    # now = self.model.parameters()
                # pre = self.model.parameters()
                # print(self.model.encoder.tcn.tcn.network[0].conv1.weight.data)
                # print(features)
                # print(loss)
