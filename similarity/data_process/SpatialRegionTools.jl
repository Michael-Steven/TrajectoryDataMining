...
function createTrainVal(region::SpatialRegion,
    ...
    h5open(trjfile, "r") do f
        trainsrc = open("$datapath/train_geolife.src", "w")
        traintrg = open("$datapath/train_geolife.trg", "w")
        trainmta = open("$datapath/train_geolife.mta", "w")

        validsrc = open("$datapath/val_geolife.src", "w")
        validtrg = open("$datapath/val_geolife.trg", "w")
        validmta = open("$datapath/val_geolife.mta", "w")
        for i = 1:ntrain+nval
            trip = f["/trips/$i"] |> read
            ...
            # println("-------------------")
            i % 50_000 == 0 && println("Scaned $i trips...")
            #i >= 8_000 && break
        end
